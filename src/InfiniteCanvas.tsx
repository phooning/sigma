...
import { ISelectionBox } from './components/SelectionBox';
...
export default function InfiniteCanvas() {
  ...
  useEffect(() => {
    ...
    const unlistenPromise = getCurrentWebview().onDragDropEvent((event: Event<DragDropEvent>) => {
      if (event.payload.type === 'drop') {
        const centerX = -viewportRef.current.x + (window.innerWidth / 2) / viewportRef.current.zoom;
        const centerY = -viewportRef.current.y + (window.innerHeight / 2) / viewportRef.current.zoom;

        const promises = event.payload.paths.map((filePath, index) => {
          return new Promise<MediaItem | null>((resolve) => {
            const lowerPath = filePath.toLowerCase();
            const isVideo = lowerPath.endsWith('.mp4') || lowerPath.endsWith('.webm') || lowerPath.endsWith('.mov') || lowerPath.endsWith('.mkv');
            ...
            if (isImage) {
              ...
              video.onloadedmetadata = () => resolve(createItem(video.videoWidth, video.videoHeight));
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    if (isPanning && startDragRef.current) {
      ...
      setSelectionBox(prev => prev ? { ...prev, endX: clientX, endY: clientY } : null);
  ...
  const handleItemPointerMove = (id: string, e: React.PointerEvent) => {
    if (draggingItem === id && startDragRef.current) {
      ...
      setItems((prev) => prev.map(item =>
        selectedItems.has(item.id) ? { ...item, x: item.x + dx, y: item.y + dy } : item
      ));
      ...
      setItems((prev) => prev.map(item =>
        selectedItems.has(item.id) ? { ...item, width: Math.max(100, item.width + dx), height: Math.max(100, item.height + dy) } : item
      ));
  ...
  const resetSize = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const target = e.currentTarget as HTMLElement;
    const mediaEl = target.parentElement?.querySelector('img, video') as HTMLImageElement | HTMLVideoElement;
  ...
  return (
    ...
    >
      <div className="canvas-background" style={{ backgroundPosition: `${viewport.x * viewport.zoom}px ${viewport.y * viewport.zoom}px`, backgroundSize: `${50 * viewport.zoom}px ${50 * viewport.zoom}px` }} />
      <div
          ...
          return (
            <div
              ...
              style={{
                ...
                width: item.width,
                height: item.height,
                zIndex: (draggingItem === item.id || resizingItem === item.id || selectedItems.has(item.id)) ? 100 : 1
              }}
            ...
            >
              <button className="reset-btn" onClick={(e) => resetSize(item.id, e)} title="Reset Size">⤡</button>
              <button className="delete-btn" onClick={(e) => deleteItem(item.id, e)} title="Delete">✕</button>
              {item.type === 'image' ? (
                <img src={item.url} alt="canvas item" draggable={false} onDragStart={(e) => e.preventDefault()} />
              ) : (
                <video src={item.url} autoPlay loop muted playsInline />
              )}
              <div className="resize-handle" onPointerDown={(e) => handleItemPointerDown(item.id, e)}></div>
            </div>
          );
        })}
      </div>
